...
while True:
    print("Здравствуйте, это программа для сортировки книг с помощью каталога")
    print("Ниже представлены все доступные команды:")
    action = input("Введите 'добавить' для добавления книги в каталог, 'показать' для просмотра каталога книг, 'удалить' для удаления книги из каталога, 'редактировать' для редактирования книги (или 'выход' для выхода): ").strip().lower()
